...
系统需要明确处理“本周既没有赚到 1000，也没有亏完 100”的情况，避免运行策略时逻辑悬空。默认策略为：到周末强制结算当前持仓，记录本周结果，下一周重置预算为 100 USDT 重新开始；不把未达标利润继续加仓复利。

**Why this priority**: OP 策略核心是“按周博弈、非复利”，若未达标周没有明确规则，系统会悄悄变成复利或无限拖延策略，偏离原始思想。

**Independent Test**: 构造 3 类周末状态（达标/亏完/未达标未亏完），系统均能产出一致且可追溯的周结算动作。

**Acceptance Scenarios**:

1. **Given** 本周收益 +300（未达 1000）, **When** 到达周结算时间, **Then** 系统强制平仓并结束本周，下一周以 100 USDT 新预算重启
...
- 周末未达标未亏完 → 强制结算并重置预算，不允许跨周复利滚动持仓
...
- **FR-026**: 当周末处于“未达标未亏完”时，系统 MUST 强制平仓并重置到下一周预算，不允许跨周复利持仓
...
- **SC-013**: 在“未达标未亏完”场景下，跨周复利持仓发生率为 0%
