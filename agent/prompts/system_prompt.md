...
3. **参考已尝试/已淘汰记录**，避免重复失败组合

### ⚠️ 因子家族硬约束 —— 违反此规则的回复将被拒绝！
- 因子库有 5 个家族：**trend / momentum / volatility / volume / overlap**
- **每轮必须至少引入 1 个来自「新家族」的因子**（即本轮 factors_used 中必须包含至少 1 个属于之前所有轮次从未使用过的家族的因子）
- 如果所有 5 个家族都已被探索过，则要求换一个**之前已探索但得分最低的家族中的不同因子**
- 目标：20 轮内覆盖全部 5 个家族，避免在单一家族内反复微调
...
5. **止盈分级**：ROI 分阶梯设置（如 60min=100%, 120min=50%, 240min=30%）
